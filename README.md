...
质量页在原始图下方显示清洗记录，两图共用时间范围和纵轴刻度；无效点留空，重新计算后更新。人工排除表位于原始图之前，输入和已有标记跟随横轴格式，可直接填写分钟或时分秒，也可使用原始图当前视图。

主分析与 MAPopt 均提供 **3、5、10、20、30、60、90、120 分钟** COx 窗口；MAPopt 可独立切换，并同时显示主分析与本图窗口。分析参数包含逐项说明及可折叠 SVG 时间示意图。每一步的“本步计算源码”可展开阅读、选择和复制该环节的正式 Python 文件，涵盖数据规范化、MAP/NIRS 质控、平均、滑动 COx、分箱与 MAPopt；源码随页面构建，历史运行需核对导出指纹。
